...
public class shootNoteVisually extends Command {
        ...
        m_swerve.setControl(
            pid_aim.withRotationalRate(
                -pid.calculate(
                    ...
                    angleHeading_new) *
                    (1.5 * Math.PI)));
        if (Math.abs(pid.getPositionError()) > VisionConstants.txTolerance) debounce_count = 0;
        else if (debounce_count < debounce_limit) debounce_count++;
        else {
          m_swerve.setControl(pid_aim.withRotationalRate(0));
          m_vision.setStage(VisionStage.SHOOT);
        }
        break;
      }
      case SHOOT: {
